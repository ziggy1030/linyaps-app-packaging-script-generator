...
build_pak() {
	...
	# 創建 wrapper 腳本
	# wrapper 位於 bin/ 目錄，使用 cd+lib 相對路徑指向 squashfs-root
...
cd "\${script_dir}/../lib/${app_prefix}"
exec ./${resolved_exec} "\$@"
WRAPPER_EOF

	chmod +x "${binary_dir}/bin/${app_prefix}.wrapper"
	echo "Created wrapper script: bin/${app_prefix}.wrapper -> lib/${app_prefix}/${resolved_exec}"
